...
- **Coverage (backfilled 2026-07-06): 106/128 fintechs have ≥1 live source** (was 6). Trustpilot 90, Google Play 68, App Store 60; **44 fintechs fully cross-platform**. App ids for mobile discovered via `npm run apify:appids` (search mode, domain-verified auto-activate; 74 trusted, 27 token-only left for manual review — see below). Backfill via `npm run apify:backfill -- <kind> <batch> [missing]`. Remaining ~22 with no source = deep long tail (no Trustpilot + no confident app match). Cross-platform insight: mobile ratings run higher than Trustpilot (Chime TP 3.5 vs GP 4.73 / AS 4.82).
- **27 token-only app-id candidates to review** (search matched by brand token, not developer domain — could be the wrong app): center, charlie, cogni, comun, coupl, curve, adro, amnis, arival, avici, hawala, hay, highbeam, hype, illimity, imagin, intera, judobank, junio, bitvavo, varo_bank, binance, bybit, gate, htx (+ dupes). Verify each app id manually, then set + activate.
